...
フロントエンドの確認は、Nix 環境から lint、型チェック、ビルド、Playwright E2E を実行できます。E2E は API レスポンスをモックするため、CI でも安定して住所補完フローを検証できます。

```bash
nix develop --command bash -c "cd frontend && yarn lint && yarn typecheck && yarn build && yarn test:e2e"
```
